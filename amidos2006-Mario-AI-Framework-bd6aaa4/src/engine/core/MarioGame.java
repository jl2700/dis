...
public class MarioGame {
    ...
    public MarioRender render = null;
    ...
    public void setupTrain(boolean visual, String level, int timer) {
        if (visual) {
            this.window = new JFrame("Mario AI Framework");
        ...
        this.world.mario.isLarge = 0 > 0;
        this.world.mario.isFire = 0 > 1;
        this.world.update(new boolean[MarioActions.numberOfActions()]);

    }
    
    public MarioResult step(int action, boolean visual) {
    	//initialize graphics
        VolatileImage renderTarget = null;
        Graphics backBuffer = null;
        Graphics currentBuffer = null;
        ...
        //while (this.world.gameStatus == GameStatus.RUNNING) {
            if (!this.pause) {
                //get actions
                agentTimer = new MarioTimer(MarioGame.maxTime);
                boolean[] actions = AgentAction.getAction(action);
                // update world
                this.world.update(actions);
                gameEvents.addAll(this.world.lastFrameEvents);
                agentEvents.add(new MarioAgentEvent(actions, this.world.mario.x,

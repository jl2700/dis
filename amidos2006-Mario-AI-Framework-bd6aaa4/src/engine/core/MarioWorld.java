...
public class MarioWorld {
    ...
    public double[][] lastFramesScreen = new double[2][121];
    ...
    public int[][] getTrainingObservation(float centerX, float centerY, int sceneDetail, int enemiesDetail) {
        int[][] ret = new int[11][11];
        int centerXInMap = (int) centerX / 16;
        int centerYInMap = (int) centerY / 16;

        for (int y = centerYInMap - 5, obsY = 0; y < centerYInMap + 5; y++, obsY++) {
            for (int x = centerXInMap - 5, obsX = 0; x < centerXInMap + 5; x++, obsX++) {
                int currentX = x;
                if (currentX < 0) {
                    currentX = 0;
                }
        ...
        for (MarioSprite sprite : sprites) {
            if (sprite.type == SpriteType.MARIO)
                continue;
            if (sprite.getMapX() >= 0 &&
                    sprite.getMapX() > centerXInMap - 5 &&
                    sprite.getMapX() < centerXInMap + 5 &&
                    sprite.getMapY() >= 0 &&
                    sprite.getMapY() > centerYInMap - 5 &&
                    sprite.getMapY() < centerYInMap + 5) {
                int obsX = sprite.getMapX() - centerXInMap + 5;
                int obsY = sprite.getMapY() - centerYInMap + 5;
                int tmp = MarioForwardModel.getSpriteTypeGeneralization(sprite.type, enemiesDetail);
                if (tmp != SpriteType.NONE.getValue()) {
                    ret[obsX][obsY] = tmp;
                }
